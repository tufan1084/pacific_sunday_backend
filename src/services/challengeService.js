...
async function checkAndAwardChallenge(userId, triggerType, metadata = {}) {
  ...
  // Run completion + wallet credit atomically so a failed credit doesn't
  // leave a completion row with no points (which would block future retries).
  try {
    await prisma.$transaction(async (tx) => {
      try {
        await tx.userChallengeCompletion.create({
          data: { userId, challengeId: challenge.id },
        });
      } catch (err) {
        if (err.code === 'P2002') {
          // Concurrent insert won the race — already completed, abort quietly.
          throw Object.assign(new Error('already_completed'), { _alreadyDone: true });
        }
        throw err;
      }

      let wallet = await tx.userPointsWallet.findUnique({ where: { userId } });
      if (!wallet) {
        wallet = await tx.userPointsWallet.create({ data: { userId, balance: 0 } });
      }
      await tx.userPointsWallet.update({
        where: { id: wallet.id },
        data: { balance: { increment: challenge.points } },
      });
    ...
    if (err._alreadyDone) return { completed: false };
    throw err;
  }
