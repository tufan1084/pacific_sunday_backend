...
const register = async (req, res, next) => {
  try {
    ...
    req.audit({
      actorId: user.id,
      actorType: 'USER',
      actorName: user.profile?.name || user.username || user.email,
      action: 'USER_REGISTER',
      category: 'AUTH',
      entityType: 'User',
      entityId: user.id,
      metadata: { email: user.email, bagUid, hasReferral: !!referralCode },
    });
  ...
  try {
    ...
    if (!user) {
      req.audit({
        ...
        category: 'AUTH',
        status: 'FAILED',
        metadata: { identifier: email, reason: 'user_not_found' },
      });
    ...
    req.audit({
      ...
      entityId: user.id,
      status: 'SUCCESS',
      metadata: { method: 'mpin', nfcUsed: !!nfcTokenRow },
    });
