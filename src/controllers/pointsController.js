...
exports.getPointsHistory = async (req, res) => {
  try {
    ...
    // Calculate totals. h2h_hold and h2h_release are reservation transactions
    // — they move heldBalance, not balance. Including them in the earned/spent
    // sums breaks the `balance == earned - spent` identity (a 10-pt hold has
    // amount=-10 but the wallet's balance never decreased). Exclude both so the
    // displayed totals reconcile with the actual balance. Settled H2H outcomes
    // (h2h_loss / h2h_winnings) DO move balance and stay in the totals.
    const allTransactions = await prisma.pointsTransaction.findMany({
      where: { userId },
    });
    const settledTransactions = allTransactions.filter(
      (t) => t.type !== 'h2h_hold' && t.type !== 'h2h_release',
    );
    ...
    // heldBalance: points reserved against open H2H challenges (the wager).
    // availableBalance: what the user can actually spend right now.
    // Surfaced separately from currentBalance so the UI can show e.g.
    // "Balance 80 • 10 on hold" without the user thinking they were charged.
    ...
    res.json({
      ...
      data: {
        summary: {
          ...
          heldBalance,
          availableBalance: Math.max(0, wallet.balance - heldBalance),
          totalEarned,
          totalSpent,
        },
